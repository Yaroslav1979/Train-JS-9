...
function getPositiveArrayValues(arr) {
  if (Array.isArray(arr)) {
    ...
    let obj = Array.from(iterator);
    let positiveArrayValues = obj.filter(element => element > 0)
...
function sumAndPrint(arr) {
  if (Array.isArray(arr)) {
    ...
    const sum = arr.reduce((accumulator, cuarentValue) => accumulator + cuarentValue);
...
function flattenSquareAndFilter(arr, threshold) {
  if (Array.isArray(arr)) {
    ...
    const squaredArray = arrFlat.map(element => element ** 2);
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr)) {
    const arrLastElement = arr.slice(-n);
    const sum = arrLastElement.reduceRight((accum, curElem) => accum + curElem);
    arrLastElement.forEach(element => console.log(element));
    return sum
    }
    return 'Вхідний параметр має бути масивом'
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function expandArrayByN(arr, n) {
  if (Array.isArray(arr) && typeof n === 'number') {
    return arr.flatMap(number => Array.from({ length: n }, () => number));
  }
...
function findDuplicateElements(arr) {
  if (Array.isArray(arr)) {
      let duplicateElements = arr.filter((element, index) => arr.indexOf(element) !== index);
